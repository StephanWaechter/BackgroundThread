...
void BackgroundThread::Thread::Start(std::function<void(void)> notifier)
{
	m_continue = true;
	m_notifier = notifier;
...
void BackgroundThread::Thread::Run(t_tasklet task)
{
	{
...
void BackgroundThread::Thread::Process()
{
	...
	while (true)
	{
		...
		currentTask(
			std::bind(
				&Thread::ForwardUiWork, this, std::placeholders::_1)
		);
	}
}